...
GENEX = "16S"
GENEY = ["potF", "recA", "gyrB", "rpoB"]
FILENAME = f"./data/data_tree_comparison_{GENEX}_GENEY.npz"
DATA_FILENAME = f"./data/significant_nodes_GENEY.tsv"

FILENAMES = [
    FILENAME.replace("GENEY", gene) for gene in GENEY
]


import pandas as pd

for jj, gene in enumerate(GENEY):
    print(f"\n------- {gene} --------")
    data = np.load(FILENAME.replace("GENEY", gene))
    nof_leafs = data["nof_leafs"]
    x = data["distance_x"]
    y = data["distance_y"]


    df = pd.read_csv(DATA_FILENAME.replace("GENEY", gene), sep="\t")
    for cc in df.columns[1:]:
        ii = np.argwhere((abs(df[cc])>4).to_numpy())[:,0]
        # idc = df["node"][ii].to_numpy()
        # print(100*len(ii)/len(x))
        print(f"{cc:15}\t{len(ii)/len(df):.3f}")
    p = plt.plot(x, y, '.', label=GENEY[jj])
    _ = plt.plot(x[idc], y[idc], 'o', mfc=[0, 0, 0, 0], mec="r")
plt.legend()
plt.show()
...
_x = data["relatedness_x"]
_y = data["relatedness_y"]
