...
FILENAME_OUT = 'seq_%s.fasta' % GENE
FILENAME_LOG = 'download_cds.log'
REMOVE_GZIP = False
REMOVE_CDS  = True
RESUME = False
MINLEN = 800
#...
FILENAME_ASSEMBLY = './CDS/REFSEQ.fna.gz'
...
for ii in range(len(names)):
    ...
    if len(header)<2 or len(body)<MINLEN:
        idlist = NCBI_search( _name+' '+GENE )
        fastas = NCBI_fetch( idlist[:5] )            
        for fasta in fastas:
            _h, _b = fasta
            if (_name.split(' ')[0] in _h) and (_name.split(' ')[1] in _h) and (GENE in _h):
                header = _h+'\n'
                body = _b
                break
        _nerrors += 1
