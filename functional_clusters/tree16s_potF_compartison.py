...
# Load all clade data
data = np.load('data_potF_all_clades.npz', allow_pickle=True)
...
# Load trees
t_16s = Phylo.read('tree_16s_labelled.newick' , 'newick')
...
#...
t_gene = Phylo.read( 'tree_potF_labelled.newick' , 'newick')
...
depth_lca_16s += np.random.randn( *depth_lca_16s.shape )/500
depth_lca_gene+= np.random.randn( *depth_lca_gene.shape )/500

plt.figure( figsize=(6,4), dpi=300)    
# plt.plot( depth_lca_16s, depth_lca_gene,'o')    
# plt.scatter( depth_lca_16s, depth_lca_gene,c = np.log10(1+np.array(n_bichos)), s=6)    
plt.scatter( depth_lca_16s, depth_lca_gene,c = np.log10(1+np.array(n_bichos)), s=6)    
