...
#...
FILENAME_GENE_SEQS = 'seq_16s_ref.fasta'
...
for clade_idx in range( len(clade_ids) ):
    ...
    for leaf_idx  in subleafs:
        ...
        if accession is not None:
            ...
            leafs_mrca_16s.append( idx_in_16s[0]  )
            leafs_mrca_gene.append(leaf_idx )            

    leafs_mrca_16s  = np.unique(leafs_mrca_16s)
    leafs_mrca_gene = np.unique(leafs_mrca_gene)
    
    
    if len(leafs_mrca_16s) > 1:
        # Relatedness in X-AXIS
        # ... find the terminal leafs of the current clade
        _clade_terminals_16s = [ leafs_16s[idx] for idx in leafs_mrca_16s ]
        ...
        d1_a = np.mean(_depth_leafs_16s)- _depth_lca_16s
        d1_b = np.min(_depth_leafs_16s) - _depth_lca_16s
        d1_c = np.max(_depth_leafs_16s) - _depth_lca_16s
        
        
        # Relatedness in Y-AXIS
        _clade_terminals_gene = [ leafs_gene[idx] for idx in leafs_mrca_gene ]
        # ... retrieve their depth
        _depth_leafs_gene = [ depths_gene[node] for node in _clade_terminals_gene ]
        # ... and find their MRCA
        lca_gene = t_gene.common_ancestor( _clade_terminals_gene )
        _depth_lca_gene = depths_gene[lca_gene]
        d2_a = np.mean(_depth_leafs_gene)- _depth_lca_gene
        d2_b = np.min(_depth_leafs_gene) - _depth_lca_gene
        d2_c = np.max(_depth_leafs_gene) - _depth_lca_gene
        
        
    else:
        d1_a, d1_b, d1_c = np.nan, np.nan, np.nan
        d2_a, d2_b, d2_c = np.nan, np.nan, np.nan
        
    
    relatedness_x.append( [d1_a, d1_b, d1_c] )
    relatedness_y.append( [d2_a, d2_b, d2_c] )
    
    nof_leafs.append( len( subleafs))
    nof_data.append( leafs_mrca_gene )
    # print('16S:  #leafs: %d \t depth: %1.4f' % ( len(leafs_mrca_16s), d1 ) )
    # print('Gene: #leafs: %d \t depth: %1.4f' % ( len(leafs_mrca_gene), d2 ) ) 
    # print('-----')

relatedness_x = np.array( relatedness_x )
relatedness_y = np.array( relatedness_y)
nof_leafs = np.array( nof_leafs )    
nof_data = np.array( nof_data)
...
if EXPORT_DATA:
    ...
    np.savez(FILENAME_OUT, nof_data = nof_data,
             nof_leafs = nof_leafs,
             relatedness_x = relatedness_x,
             relatedness_y = relatedness_y)

plt.plot( relatedness_x[:,0], relatedness_y[:,0],'.')
