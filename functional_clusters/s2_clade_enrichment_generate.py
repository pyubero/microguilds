...
FILENAME_OUT = 'data_clade_enrichment_potF.npz'
MCMAX = 9999 ## 999 takes 220s
...
#             name = ' '.join( line[1:3])
#             acc2names.update( {accession : name} ) 
# genus_list   = np.array( [acc2names[name].split(' ')[0] for name in leaf_names] )
# species_list = np.array( [acc2names[name].split(' ')[1] for name in leaf_names] )
...
_missing_data = (np.sum(np.all( np.isnan(F), axis=1)))
_not_missing_data= F.shape[0] - _missing_data
print('')
print('Data obtained for %d tree entries.' % _not_missing_data)            
print('Data missing  for %d tree entries.' % _missing_data)            
